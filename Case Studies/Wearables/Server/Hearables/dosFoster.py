...
def changeSecurityState(state):
	global secure
	    ...
	    secure = False
            ...
            secure = True
            label.config(text="Normal", background="green", foreground="black")

        if not secure:
            multRes = [[1 for x in range(3)] for y in range(5)]  
            print(os.getcwd())
            os.chdir("heartmonitorpns/HM_R1_A1/src/")
            for i in range(3):
                for j in range(5):
                    if i > 0 and j > 0:
                        print(os.getcwd())
                        os.chdir("../../HM_R"+ str(i) + "_A" + str(j) + "/src/")
                        os.system("java HM" )
                        #call(["java", "HM"])
                        print("Ran program")
            ...
            if result[2] >= result[1] and result[2] > result[3] and result[2] > result[4]:
                #startFosterServer()\
                print("Activate Fostering Server and Empty Packets")
                thread.start_new_thread(startFosterServer,(30,))
            elif result[3] > result[1] and result[3] > result[2] and result[3] > result[4]:
                print("Disconnecting")
                disconnectDevice()
            elif result[4] > result[1] and result[4] > result[2] and result[4] >= result[3]:
                print("Activate Fostering Server and Disconnecting")
                disconnectDevice()
                thread.start_new_thread(startFosterServer,(30,))
            else:
                print("Sending Empty Packets, do not allow fostering")
            os.chdir("../../../")
            print(os.getcwd())
...
def fosterWithServer():
    ...
    while True:
		...
		for j in range(16):
			if j*2 < 10:
				uuid = "2e0ca4ea-299d-4335-93eb-27fcfe7fa80" + str(j*2)
			else:
        			uuid = "2e0ca4ea-299d-4335-93eb-27fcfe7fa8" + str(j*2)
        		service_matches = find_service( uuid = uuid, address = addr )
        		for k in service_matches:
        	    		print k
        			if len(service_matches) == 0:
        	    			print("couldn't find the SampleServer service =( " + uuid)
        			if len(service_matches) > 0:
        	    			thread.start_new_thread(fosterConnect, (k,))
        	    			time.sleep(3)
		    		break
...
def fosterConnect(match):
	...
	#Get spawn server to listen for infoi
		...
		if portNumber < 10:
			uuid = "2e0ca4ea-299d-4335-93eb-27fcfe7fa80" + str(portNumber)
...
def startServer(portNumber):
	...
	print("Starting loop")
        client_sock,address = server_sock.accept()
        print("after server accept in startServer")
        f = open("test.txt", "r")
        ...
        try:
            while True:
                ...
                if len(data) > 0:
                    print data
                if "State: True" in data and data in "State: True" and not secure:
                    toldFalse = False
                    changeSecurityState(data)
                    hat.show_letter("S", text_colour=[0,255,0])
                if "State: False" in data and data in "State: False" and secure:
                    print("told to change security state")
                    toldFalse = True
                    hat.show_letter("U", text_colour=[255,0,0])
	            changeSecurityState(data)
                    
                if "State: Disconnect" in data and data in "State: Disconnect":
                    hat.show_letter("U", text_colour=[255,0,0])
                    disconnectDevice()
                    client_sock.close()
                    break
                if "State: Reconnect" in data and data in "State: Reconnect":
                    print("In Reconnect")
...
def connect(match):   
    ...
    while True:
        ...
        if secure and not getDisconnected():
                ...
                for i in sharedBuffer:
		    print(sharedBuffer[i])
                    sock.send(sharedBuffer[i])
...
label = tk.Label(root,text="Connecting to Base Station", width=40,  height=4) 
